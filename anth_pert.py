...
def recon(args):
    ...
    save_dir = f'results/{args.exp_name}/test/recon-ap/{args.epoch}'
    plot_dir = f'results/{args.exp_name}/test/recon-ap/{args.epoch}/plot'
    ...
    print("... Test start")
    model.eval()
    ITS = np.linspace(0,1,5)
    for intensity in ITS:
        RMSE = []
        SD = []
        for i, ts in enumerate(test_loader):
            ...
            an_mes = an_mes + intensity * an_mes * torch.randn_like(an_mes)
            ...
            for az in range(sel_grid):
                ...
                rmse = F.mse_loss(estimate, target_m).pow(.5)
                sd = F.l1_loss(estimate, target_m)
            ...
            if (args.x_constraint is not None) or (args.y_constraint is not None) or (args.z_constraint is not None):
                ...
                for b in range(B):
                    ...
                    plt.savefig(f'{plot_dir}/{args.x_constraint}-{args.y_constraint}-{args.z_constraint}-{i}-{b}-{intensity}.png')
                    plt.close()
        with open(log_path,'a') as log:
            log.write(f"[Test] Intensity {intensity} \n")
            log.write(f"       RMSE: {args.rescale * sum(RMSE)/len(testset)} dB\n")
            log.write(f"       SD  : {args.rescale * sum(SD)/len(testset)} dB\n")
        print(f"[Test] {intensity} ")
        print(f"       RMSE: {args.rescale * sum(RMSE)/len(testset)} dB")
